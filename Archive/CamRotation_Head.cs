...
public class CamRotation_Head : MonoBehaviour
{
    ...
    private float HMD_Xori, HMD_Yori, HMD_Zori;
    ...
    void Start()
    {
        ...
        //string logPath = String.Concat(currentPath, "\\log");
        //string offlineLogPath = String.Concat(logPath, "\\offline");
        ...
        //if (!Directory.Exists(offlineLogPath))
        //{
        //    Directory.CreateDirectory(offlineLogPath);
        //    Directory.CreateDirectory(String.Concat(offlineLogPath, "\\PRTS"));
        //    Directory.CreateDirectory(String.Concat(offlineLogPath, "\\SoS"));
        //    Directory.CreateDirectory(String.Concat(offlineLogPath, "\\PRBS"));
        //}
        ...
        AnklePosition = new Vector3(transform.position.x, transform.position.y - 1.25f, transform.position.z);

        // ******************  EXPERIMENTS SETUP  ******************
        flagExpNum = 8;  // 0: Normal VR  1: Disabled Head-tracking  2: Magnify Rotation  3: Step-response (5 and 10 deg.) 4: Sinusoidal Perturbations 
        ...
        SS_freqCount = 20; // 30 Fcount = 1.5Hz
        ...
        TrapZ_minDeltaSec = 2.5f; //Wait Time this is minDelta. delta = minDelta + rand(0, minDelta)
        TrapZ_minWSec = 2.5f; //Pulse Width, this is w. W = w + rand(0, w)

        TrapZ_P2PAmp = 10.0f; //deg                                        <<========================================
        TrapZ_Amp = TrapZ_P2PAmp/2.0f; //degrees
        TrapZ_Vel = 5.0f; //deg per sec                                   <<========================================
        ...
        dataLogger = new StreamWriter("log/VR_" + logFileName + dateString + ".txt");
        timeLog = new StreamWriter("log/Time_" + logFileName + dateString + ".txt");
        daqSignal = new StreamWriter("log/daq_" + logFileName + dateString + ".txt");
        headMotionLog = new StreamWriter("log/HeadMotion_" + logFileName + dateString + ".txt");
    ...
    void FixedUpdate()
    {
        ...
        HMD_Xori = trackingInfo.headOriX_local;
        HMD_Yori = trackingInfo.headOriY_local;
        HMD_Zori = trackingInfo.headOriZ_local;

        if (HMD_Xori > 180) HMD_Xori = HMD_Xori - 360;
        if (HMD_Xori < -180) HMD_Xori = HMD_Xori + 360;

        if (HMD_Yori > 180) HMD_Yori = HMD_Yori - 360;
        if (HMD_Yori < -180) HMD_Yori = HMD_Yori + 360;

        if (HMD_Zori > 180) HMD_Zori = HMD_Zori - 360;
        if (HMD_Zori < -180) HMD_Zori = HMD_Zori + 360;
                ...
                case 10: //TrapZ - Randomized
                    {
                        ...
                        WriteLogHeadTracking(HMD_Xori, HMD_Yori, HMD_Zori);
                        xEulerPrevious = xEuler;
    ...
    public void WriteLogHeadTracking(float OriX , float OriY, float OriZ)
    {
        headMotionLog.WriteLine();
        headMotionLog.Write(OriX + ",");
        headMotionLog.Write(OriY + ",");
        headMotionLog.Write(OriZ + ";");
    }
