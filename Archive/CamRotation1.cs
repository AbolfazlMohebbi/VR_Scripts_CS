...
public class CamRotation1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // ******************  EXPERIMENTS SETUP  ******************
        flagExpNum = 9;  // 0: Normal VR  1: Disabled Head-tracking  2: Magnify Rotation  3: Step-response (5 and 10 deg.) 4: Sinusoidal Perturbations 
                          // 5: PRBS  6: Randomized Step    7: Randomized Sinusoidal    8: Sum of Sinusoids    9:PRTS    10:TrapZ    11: TrapzV  
                          //  12:SumofSin not Normalized
        ...
        SS_result = InputFunctions.SumOfSin_offline(3, SS_Amplitude, 0.05f, 0.1f, 2, out SS_TimeTotal, SS_dtVR, false).ToArray();
        ...
        //TrapzV_Pos = new float[TrapzV_nPulses * dt_Count];
        //TrapzV_Pos = TrapzVel(TrapzV_nPulses, TrapzV_ti, TrapzV_ta, TrapzV_tv, TrapzV_tx, TrapzV_dtVR, TrapzV_maxVel);
    ...
    void FixedUpdate()
    {
        ...
        if ((Time.time > Time_0) && (Time.time < Time_F) )  //Perturbation start and finish time
        {
            ...
            switch (flagExpNum)
            {
                ...
                case 12: //Sum of Sinusoids Perturbations - Not Normalized
                    {
                        sinSum = 0.0f;
                        daqInput = 0.0f;
                        if (Counter == 0)
                ...
                case 13: //Randomized Half Sinusoidal Perturbations wrt ankle
                    {
                        if (Counter == 0)
                        {
                            xEulerInit = trackingInfo.headOriX;
    ...
    public float[] TrapzVel(int NumOfPulses, float ti, float ta, float tv, float tx, float timeRes, float VelocityAmp)
    {
        ...
        for (int i = 0; i < NumOfPulses; i++)
        {
            ...
            for (int k = 0; k < dt_Count; k++)
            {
                ...
                TrapzV_posFile.WriteLine(PositionProfile[i * dt_Count + k] + ",");
                TrapzV_velFile.WriteLine(VelocityProfile[i * dt_Count + k] + ",");
            }
        }
